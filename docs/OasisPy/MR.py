...
def MR(path, method='swarp', sig_thresh=4, gauss_sig=3, gauss_filt=False, use_config_file=True):
    ...
    :param str method: Stacking method. 
        ...
        * *sos_abs*: Absolute sum of squares, pixel-wise. Preserves sign. Mathematically, this look like :math:`\Sigma(p_i \cdot |p_i|)` with :math:`p_i` being the :math:`ith` pixel. For example, a series of pixels [10, 2, -3, -6] would be stacked according to 100 + 4 + -9 + -36.
    ...
    :param float sig_thresh (default=4): Only used for *sigma_clip* method. Number of sigmas pixel must exceed to be used as stacked value.
    :param float gauss_sig (default=3): Only used for *sigma_clip* method. Number of sigmas used for gaussian filter.
    :param bool gauss_filt (default=False): Only used for *sigma_clip* method. When ``True`` the final master residual will be smoothed with a gaussian filter with a sigma equal to *gauss_sig*.
    :param bool use_config_file (default=True): If ``True`` all input parameters are fetched from the local *OASIS.config* file.
