...
#    print('\t Modified for OASIS Pipeline by: Andrew Stewart (andrew.henry.stewart@emory.edu)')
#    print('\t ----------------------------------------------\n')
...
#    f = open('userdata.dat', 'r')
    ...
    if ending_date == "":
        ...
        if int(c_d) + 1 <= calendar.monthrange(int(c_y), int(c_m))[-1]:
            ...
            ending_date = str(int(c_y) + 1) + '-01-01'
#    obj = input("-> Object name (default= any object): ")
#    fltr = input("-> Filter (default= any filter): ")
#    exptime = input("-> Exposure time (default= any exposure time): ")
#    siteid = input("-> Site ID (default= any site ID): ")
#    telid = input("-> Telescope ID (default= any telescope ID): ")
#    if obj == '':
#        obj = '{}'
#    if fltr == '':
#        fltr = '{}'
