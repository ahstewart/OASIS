...
#            weightMap = weight_map(MR)
#            weight_hdu = fits.PrimaryHDU(weightMap)
#            weight_map_loc = location + '/configs/weight_map.fits'
#            weight_hdu.writeto(weight_map_loc)
            with open(config_loc, 'r') as config:
                data = config.readlines()
                config.close()
...
#    print("-> Masking filtered sources...")
#    for i in images:
#        filters.mask_sources_image(i)
...
#    length = len(residuals) + 1
    ...
    filter_sources(location)
#    print("-> Masking filtered sources...")
#    filters.mask_sources_image(image)
...
def filter_sources(location):
    ...
    print("-> Filtering out repeated detections...")
    filters.reoccuring(location)
...
def mv_refs(mv_loc, locs=[]):
    for l in locs:
        ref = glob.glob("%s/data/*_ref_A_.fits" % (l[0]))
        os.system("cp %s %s/%s_ref_A_.fits" % (ref[0], mv_loc, l[1]))
