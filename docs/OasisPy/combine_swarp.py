...
def swarp(location):
    ...
    if len(temps) == 0 or numImages != len(images):
        ...
        print("-> FOV size (x,y): (%d, %d)" % (correct_shape[0], correct_shape[1]))
        ...
        #stack all input image masks to form the template mask
        #template mask is formed pixel by pixel
        #if a pixel has 50% or more of its values masked, the template mask
        #pixel will be masked
#        print("\n-> Combining image masks into template mask...")
#        imMasks = []
#        for i in images:
#            hdu = fits.open(i)
#            imMasks.append(hdu[1].data)
#            hdu.close()
#        imMasks = np.array(imMasks)
#        imMasksSum = imMasks.sum(axis=0)
#        imMasksSum = (imMasksSum/len(images)).astype(np.float32)
#        mask = (imMasksSum>=0.50).astype(int)
#        for x in range(correct_shape[0]):
#            for y in range(correct_shape[1]):
#                pixels = []
#                for i in images:
#                    hdu = fits.open(i)
#                    hduMask =  hdu[1].data
#                    pixels.append(hduMask[x,y])
#                    hdu.close()
#                pixels = np.array(pixels)
#                ones_ratio = float(((pixels==1).sum())/(len(pixels)))
#                if ones_ratio >= 0.50:
#                    mask[x,y] = 1
#                else:
#                    mask[x,y] = 0
