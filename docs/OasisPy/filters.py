...
def get_sources(image, filtered=True, MR=False):
    #gets all the point sources and fluxes deteced with SExtractor on a certain
    #residual image
    #the format of the outputted data is first column: flux, second: x pixel
    #position, third: y pixel position
...
def reoccuring(location):
    ...
    for i in range(len(sources)):
        ...
        for j in range(len(new_sources[i])):
            inds = []
            x_low = round(new_sources[i][j][1]) - 1.5
            x_high = round(new_sources[i][j][1]) + 1.5
            y_low = round(new_sources[i][j][2]) - 1.5
            y_high = round(new_sources[i][j][2]) + 1.5
            check = 0
...
def variable_stars(location):
    #get location of template image
    temps = glob.glob("%s/templates/*.fits" % (location))
    if len(temps) != 1:
        print("-> Error: problem with nbumber of template images\n-> Exiting...")
        sys.exit()
    else:
        template = temps[0]
    #get FWHM and psf of template
    template_fwhm = psf.fwhm_template(template)
    template_psf = template.replace('templates', 'psf')
    template_psf = template_psf.replace('fits', 'psf')
    #make source catalog of template using SExtractor
    sim_config = "%s/configs/default_sim.sex" % (location)
    sim_params = "%s/configs/default_param_sim.sex" % (location)
    with  open(sim_config, 'r') as conf:
        lines = conf.readlines()
    lines[6] = "CATALOG_NAME" + "        " + "%s/templates/template.cat" % (location) + "\n"
    lines[9] = "PARAMETERS_NAME" + "        " + sim_params + "\n"
    lines[22] = "FILTER_NAME" + "        " + "%s/configs/default.conv" % (location) + "\n"
    lines[70] = "SEEING_FWHM" + "        " + str(template_fwhm) + "\n"
    lines[127] = "PSF_NAME" + "        " + template_psf + "\n"
    with open(sim_config, 'w') as conf_write:
        conf_write.writelines(lines)
    os.system("sextractor %s -c %s" % (template, sim_config))
    #get object centroid locations from catalog
    template_cat = "%s/templates/template.cat" % (location)
    with open(template_cat, 'r') as cat:
...
def spread_model_filter(location, MR=False):
    #filter source file by spread_model and puts the results in
    #filtered_sources.txt
...
def mask_sources_image(res_image, aperture_radius=2):
    ...
    for unf in unfiltered_sources:
        if unf not in filtered_sources:
            new_mask = mask_source(res_data.shape[0], res_data.shape[1], (unf[1], unf[2]), aperture_radius*im_fwhm)
...
def source_profile(res_image, x_pos, y_pos, aperture_radius=2):
    res_data = fits.getdata(res_image)
    image = res_image.replace('_residual', '')
    image = image.replace('residuals', 'data')
    image_fwhm = psf.fwhm(image)
    aperture = int(image_fwhm * aperture_radius)
    x_profile = res_data[round(x_pos), round(y_pos) - aperture:round(y_pos) + aperture + 1]
    y_profile = res_data[round(x_pos) - aperture:round(x_pos) + aperture + 1, round(y_pos)]
